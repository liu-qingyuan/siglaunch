...
- Recognition Success 后，Primary Workflow 的第一个外部 Effect 是只读查询 Herdr Agent 列表，发生在读取配置或触碰 Ghostty 之前。
- 完整解码的 snapshot 中只要存在精确的 `agent == "pi"`，就以 Pi Agent Preserved 成功结束；Workspace、`cwd`、`foreground_cwd`、返回顺序、pane 和 focused 状态都不参与判断，也不执行 focus。
- Pi Agent Preserved 分支不加载 Workflow 配置、不解析、启动或激活 Ghostty、不确保 Herdr GUI Session，也不启动进程；前台应用、Ghostty 与 Herdr focus 保持不变。
- 初次 snapshot 为空或仅含非 Pi Agent，或初次 Herdr 不可用时，才进入 cold start；初次输出 malformed 时在配置和终端副作用前失败。
- Cold start 先加载并验证配置，再检查 Ghostty 是否已安装并正在运行；未安装时停止并报告错误，未运行时先启动 Ghostty。
...
- Siglaunch 确保 Ghostty 中连接的是 Herdr 默认 Session；若默认 Session 尚未运行，则在 Ghostty 中启动 `herdr`。ADR 0003 的 terminal automation 只适用于这条 cold-start 路径。
- 受管 Herdr Session ready 后再次查询 Agent；此时可见 Pi Agent则保留，完整 snapshot 中仍无 Pi 才由 Herdr 在配置的 Workspace 启动 Pi。Post-bootstrap 查询不可用或 malformed 时失败且不启动 Pi。
- 第二次 snapshot 只避免启动其中已经可见的 Pi；query 与 start 之间仍有 TOCTOU race，本轮不增加锁、reservation 或原子 find-or-start contract。
...
- MVP 写死 Domain Expansion Pose 的两阶段识别管线、`3/5` 触发规则、query-first 的 Pi Agent Preserved 判断、cold start 中 Ghostty 到默认 Herdr Session 再到 Pi Agent 的步骤顺序，以及 Domain Expansion HUD。
